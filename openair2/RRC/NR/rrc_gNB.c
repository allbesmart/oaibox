...
//-----------------------------------------------------------------------------
{
  ...
  *DRB_config->pdcp_Config->t_Reordering = NR_PDCP_Config__t_Reordering_ms100;
  ...
  int16_t size = do_RRCReconfiguration(ctxt_pP,
                                       buffer,
                                       RRC_BUF_SIZE,
                                       xid,
...
//-----------------------------------------------------------------------------
void rrc_gNB_generate_RRCReestablishment(const protocol_ctxt_t *const ctxt_pP,
                                         rrc_gNB_ue_context_t *const ue_context_pP,
...
//-----------------------------------------------------------------------------
{
  ...
  uint8_t buffer[RRC_BUF_SIZE];
  int16_t size = do_RRCReestablishment(ctxt_pP,
                                       ue_context_pP,
                                       CC_id,
                                       buffer,
                                       RRC_BUF_SIZE,
  ...
  if (!NODE_IS_CU(rrc->node_type)) {
    ...
    nr_mac_enable_ue_rrc_processing_timer(ctxt_pP->module_id, ue_context_pP->ue_context.rnti, *rrc->carrier.servingcellconfigcommon->ssbSubcarrierSpacing, delay_ms);
  }
...
{
  uint8_t buffer[RRC_BUF_SIZE];
  int8_t size = do_NR_RRCRelease(buffer, RRC_BUF_SIZE, rrc_gNB_get_next_transaction_identifier(ctxt_pP->module_id));
  ue_context_pP->ue_context.ue_reestablishment_timer = 0;
  ue_context_pP->ue_context.ue_release_timer = 0;
  ue_context_pP->ue_context.ul_failure_timer = 0;
